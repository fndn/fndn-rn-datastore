...
var Data 	 			= {};
Data.tables 			= {}; 		// memorymapped async store
...
var all = module.exports.all = (table) => {
	console.log('% all', table);
	return Data.tables[table] ? Data.tables[table].findAll() : [];
}
...
// find where
var where = module.exports.where = (table, query) => {
	console.log('% where', table, query);

	if( !Data.tables[table] ){
		console.log('% where 0: table not found ('+ table +')' );
		return false;
	}else{
		return Data.tables[table].where(query).find();
	}
}
...
function _announceChange(table){
	...
	for( var fn in _subscribers[table] ){
		...
		_subscribers[table][fn]( Data.last(table) );
	}
