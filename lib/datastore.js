...
function _process_init_queue(){
	...
	if( _init_queue.length ){
		...
		for(var fn in _init_queue ){
			console.log('[Datastore] Calling Queued fn');
			_init_queue[fn]();
		}
		_init_queue = [];
	}
...
var addu = module.exports.addu = (table, val) => {
	...
	if( !Data.tables[table] ){
		...
		if( r.length > 0 ){
			...
			//console.log('% addu 3: creating record', r);
...
var put = module.exports.put = (table, key, val) => {
	...
	if( !Data.tables[table] ){
		...
		//Data.tables[table].updateById( r._id, r); /// _!_
		r = Data.tables[table].where(key).update(r);
...
var del = module.exports.del = (table, key) => {
	...
	if( !Data.tables[table] ){
		...
		if( r.length == 0 ){
			...
			var val = Data.tables[table].where(key).remove();
			_announceChange(table);
			return {removed_id: val};
		}
	}
}
	...
	if( !Data.tables[table] ){
		...
		if( r.length == 0 ){
			...
			r.forEach( function(ar){
				...
				ids.push( Data.tables[table].where(key).remove() );
			});
...
module.exports._forceAnnounce = function(table){
	_announceChange(table);
}
