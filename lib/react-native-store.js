...
//var ObjectCompare = require('./object.compare.js');
...
function removeIDs(obj){
	...
	var robj = {}, keys = Object.keys( obj ), kl = keys.length;
	for(var k=0; k<kl; k++) if( keys[k] != '_id' ) robj[keys[k]] = obj[keys[k]]; 
	...
	if (hasParams) {
		for (var row in rows) {
			...
			for (var key in this._where) {
				...
				/*
				// js test, to ignore _id in matches:
				var a = rows[row][key];
				var b = this._where[key];
				console.log('0 comparing ', a, " to ", b, "result:", (a==b) );
				var x = removeIDs(a);
				var y = removeIDs(b);

				console.log('1 comparing ', x, " to ", y, "result:", (x==y) );
				//console.log('2 comparing ', x, " to ", y, ObjectCompare(x,y) );
				console.log('3 comparing ', x, " to ", y, "result:", Object.is(x,y) );
				
				isMatch = Object.is(x,y);
				*/
				// org
