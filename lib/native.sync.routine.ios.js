...
module.exports.Run = function( progress_cb, completion_cb, mode ){
	...
	if( _mode == "sync" ){
		...
		//_tables = ["countries", "locations"];
		//_tables = ["countries", "locations", "brands", "incomeTypes", "storeTypes", "storeBrands", "ageGroups"];
...
function _upload_next(){
	...
	if( _upl_step < _upl_steps ){
		...
		Datastore.data.all("registrations", function(items){	
...
function _upload_item_uploaded( _item, _response ){
	...
	Datastore.data.put(_upl_table, _item._id, _item);
...
function _check(_table){
	...
	*/
	...
	Datastore.data.all(_table, function(items){
		...
		/// cleanup items before sending (should be moved to UI code)
		var clean_items = [];
		for(var i=0; i<items.length; i++){
			var o = Datastore.clone(items[i]);
			//console.log('CLEAN IN:', o);
			if( o._id ) delete o._id;

			if( o.hash == undefined ){
				var str = JSON.stringify(o);
				o.hash = Datastore.md5( str );
				console.log('==================== CREATING hash:', o, str );
			}

			clean_items.push( o );
			//console.log('CLEAN OUT:', o);
		}
		console.log('%% local items in table, cleaned', _table, ":", clean_items);
